...
<body>
    ...
    <!-- CONTEXT MENU (DİNAMİK) -->
    <div id="context-menu">
        <div class="ctx-header" id="ctx-header-text">KOMUTA MENÜSÜ</div>
        <!-- Yıldız Menüsü -->
        <div id="ctx-star-options" style="display:none;">
            <div class="ctx-item" id="ctx-jump"><span class="ctx-icon">🚀</span> HYPER-JUMP</div>
            <div class="ctx-item" id="ctx-analyze"><span class="ctx-icon">📊</span> YILDIZ ANALİZİ</div>
        </div>
        <!-- Boşluk Menüsü -->
        <div id="ctx-space-options" style="display:none;">
            <div class="ctx-item" id="ctx-scan-sector"><span class="ctx-icon">📡</span> SEKTÖR TARAMASI</div>
            <div class="ctx-item" id="ctx-mark-coord"><span class="ctx-icon">📍</span> KOORDİNAT KAYDI</div>
        </div>
        <div class="ctx-item danger" id="ctx-cancel"><span class="ctx-icon">❌</span> KAPAT</div>
    </div>

    <!-- KALICI GÖZ BUTONU -->
    <button id="global-toggle-btn" title="Arayüzü Aç/Kapat">👁️</button>
    ...
    <div id="start-screen">
        ...
        <button class="start-btn" id="btn-init">BAĞLANTIYI KUR</button>
    </div>

    <!-- Bildirim -->
    <div id="message-overlay">WARP SÜRÜŞÜ AKTİF</div>
    ...
    <div id="ui-layer">
        ...
        <div class="hud-panel top-center-mission">
            <div class="mission-header">AKTİF GÖREV</div>
            <div class="mission-text" id="mission-desc">BEKLEMEDE...</div>
            <div class="mission-xp" id="player-rank">RÜTBE: ACEMİ PİLOT</div>
        </div>

        <!-- Tarayıcı -->
        <div class="hud-panel top-left-scanner">
            <div class="scanner-title">TARAMA MODÜLÜ</div>
            <div class="scanner-data">
                ...
                <div class="scanner-row" style="border:none; margin-top:5px;"><span>DURUM:</span> <span class="scanner-val" id="net-status" style="color:var(--holo-green)">PASİF</span></div>
            </div>
            <div class="target-locked" id="lock-msg">HEDEF KİLİTLENDİ<br><span style="font-size:0.8em; color:#fff; font-weight:normal;">(Sağ Tık: Menü)</span></div>
        </div>

        <!-- Başlık -->
        <div class="hud-panel top-right">
            <h1>SAMANYOLU</h1>
            <div class="subtitle">V7.6 KOKPİT</div>
        </div>
        ...
        <div class="hud-panel bottom-center">
            ...
            <div class="control-group" style="border:none;">
                ...
                <button class="control-btn" id="btn-hide">GİZLE</button>
            </div>
        </div>
        <div class="controls-help">SOL TIK: SEÇ | SAĞ TIK: MENÜ | SÜRÜKLE: DÖNDÜR</div>
        ...
        <div class="hud-panel right-center-terminal">
            <div class="terminal-title">GÖREV & ANALİZ KAYITLARI</div>
            <div class="terminal-content" id="terminal-out">
                <div class="term-line">Sistem hazır...</div>
            </div>
        </div>
    </div>
    ...
    <script type="module">
        ...
        let lockedTargetPos = null; // Kilitlenilen yıldızın pozisyonu
        let rightClickPos = null; // Sağ tıklanılan herhangi bir koordinat (boşluk için)
        ...
        let playerRank = 1;
        const ranks = ["ACEMİ PİLOT", "YILDIZ GEZGİNİ", "GALAKTİK KAŞİF", "UZAY EFSANESİ"];
        ...
        function updateAudioUI() {
            if (isMuted) {
                ...
                elAudioText.innerText = "PASİF";
                elAudioText.style.color = "var(--alert-red)";
            } else {
                btnMute.innerText = "🔊";
                btnMute.classList.remove('muted');
                elAudioText.innerText = "AKTİF";
        ...
        function init() {
            ...
            window.addEventListener('click', (e) => {
                ...
                // Eğer canvasa tıklandıysa seçim yap
            ...
            window.addEventListener('contextmenu', (e) => {
                ...
                if (e.target.tagName === 'CANVAS' && !isWarping) {
                    ...
                    // Tıkladığımız yerde bir yıldız var mı kontrol et
                    ...
                    if(intersects.length > 0 && intersects[0].distanceToRay < 0.2) {
                        // YILDIZ MODU
                        headerText.innerText = "YILDIZ PROTOKOLÜ";
                        starOptions.style.display = 'block';
                        spaceOptions.style.display = 'none';
                        
                        // Kilitlenilen yıldız pozisyonunu güncelle (eğer sol tıklanmadıysa bile sağ tıkla seç)
                        ...
                        // BOŞLUK MODU
                        headerText.innerText = "SEKTÖR KOMUTASI";
                        starOptions.style.display = 'none';
                        spaceOptions.style.display = 'block';
                        
                        // Boşluk pozisyonu için basit bir Raycast düzlemi hayal edebiliriz
                        // Şimdilik sadece kameradan bir yön vektörü alıyoruz
            ...
            document.getElementById('ctx-analyze').onclick = () => {
                ctxMenu.style.display = 'none';
                if(lockedTargetPos) {
                    logToTerminal(`YILDIZ ANALİZİ BAŞLATILDI...`, 'analysis');
                    setTimeout(() => logToTerminal(`> KÜTLE: ${(Math.random()*5+0.5).toFixed(2)} Güneş Kütlesi`), 500);
                    setTimeout(() => logToTerminal(`> YAŞ: ${(Math.random()*10).toFixed(1)} Milyar Yıl`), 1000);
            ...
            document.getElementById('ctx-scan-sector').onclick = () => {
                ...
                logToTerminal(`BÖLGE TARANIYOR...`, 'analysis');
                ...
                setTimeout(() => logToTerminal(`> SİNYAL KAYNAĞI: YOK`), 1800);
            };
            document.getElementById('ctx-mark-coord').onclick = () => {
                 ctxMenu.style.display = 'none';
                 logToTerminal(`KOORDİNATLAR KAYDEDİLDİ.`, 'highlight');
            };
        ...
        function createNewMission() {
            const types = ['M (KIZIL)', 'O (MAVİ)', 'G (SARI)'];
            const targetType = types[Math.floor(Math.random() * types.length)];
            currentMission = { targetType: targetType, description: `TARAMA GÖREVİ: Bir ${targetType} yıldız bul.` };
            elMissionDesc.innerText = currentMission.description;
            elMissionDesc.style.color = "#fff";
            logToTerminal(`YENİ GÖREV ALINDI: ${targetType} Bul`, 'highlight');
        }
        ...
        function checkMissionCompletion(starClass) {
            ...
            if (starClass === currentMission.targetType) {
                ...
                elOverlay.innerText = "GÖREV BAŞARILI!"; elOverlay.style.color = "var(--gold)"; elOverlay.style.opacity = 1;
                setTimeout(() => { elOverlay.style.opacity = 0; elOverlay.style.color = "#fff"; }, 2000);
                logToTerminal(`GÖREV TAMAMLANDI: ${starClass}`, 'mission-update');
                playerRank++; updateRankUI(); setTimeout(createNewMission, 3000);
            }
        }
        function updateRankUI() {
            let rankTitle = ranks[Math.min(playerRank - 1, ranks.length - 1)];
            elPlayerRank.innerText = `RÜTBE: ${rankTitle} (SEVİYE ${playerRank})`;
        }
        ...
        function updateTerminal() {
            ...
            if(rand > 0.98) div.innerText = `[SİSTEM] Veri Akışı: ${Math.floor(Math.random()*999)} Mb/s`;
        ...
        function updateConnections() {
            if(!lockedTargetPos || isWarping || currentMode === 'nav') {
                connectionLines.visible = false; elNetStatus.innerText = "PASİF"; return;
            }
            connectionLines.visible = true; elNetStatus.innerText = "ANALİZ EDİLİYOR...";
            ...
            elNetStatus.innerText = `BAĞLANTI: ${count} DÜĞÜM`;
        }
        ...
        function onMouseClick(e) {
            if(lockedTargetPos && !isWarping && currentMode === 'scan') {
                // Tıklama sadece ses efekti versin, zıplamayı menüye taşıdık
        ...
        function toggleWarp() {
            ...
            if (isWarping) {
                ...
                elOverlay.innerText = "WARP HIZI AKTİF"; elOverlay.style.opacity = 1; setTimeout(() => elOverlay.style.opacity = 0, 2000);
            } else {
        ...
        function checkInteractions() {
            // Navigasyon modunda veya obje yoksa işlem yapma
            ...
            if (intersects.length > 0) {
                ...
                if (point.distanceToRay < 0.2) {
                    ...
                    if(temp > 8000) { starClass = 'O (MAVİ)'; color='#00ffff'; }
